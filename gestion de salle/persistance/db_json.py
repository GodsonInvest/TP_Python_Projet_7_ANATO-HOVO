"""
Couche de persistance JSON — module json (bibliotheque standard Python).

Format alternatif a SQLite.
...
class BaseDonneesJSON:
    ...
    Chaque entite est stockee dans un fichier JSON distinct.
    Les fichiers sont crees automatiquement au premier enregistrement.
    """
    ...
    def _lire(self, chemin: str) -> list:
        """Charge un fichier JSON et retourne sa liste. Retourne [] si absent."""
    ...
    def _ecrire(self, chemin: str, donnees: list):
        """Ecrit une liste de dicts dans un fichier JSON (indenté, UTF-8)."""
    ...
    def sauvegarder_utilisateur(self, utilisateur: Utilisateur) -> int:
        """
        Ajoute ou met a jour l'utilisateur dans utilisateurs.json.
        Attribue un id si l'utilisateur n'en a pas encore.
        Retourne l'id.
        """
    ...
    def _dict_vers_utilisateur(d: dict) -> Utilisateur:
        """Reconstruit un Utilisateur depuis un dictionnaire JSON."""
        role = d["role"]
        mdp  = d["mot_de_passe"]

        if role == "etudiant":
            u = Etudiant(
                d["nom"], d["email"], d["login"], mdp,
    ...
    def sauvegarder_salle(self, salle: Salle) -> int:
        """
        Ajoute ou met a jour la salle dans salles.json.
        Attribue un id si la salle n'en a pas encore.
        Retourne l'id.
        """
    ...
    def sauvegarder_reservation(self, reservation: Reservation) -> int:
        """
        Ajoute ou met a jour la reservation dans reservations.json.
        L'id est celui attribue par Reservation._compteur.
        Retourne l'id.
        """
    ...
    def _dict_vers_reservation(
        ...
        if salle is None or responsable is None:
            raise ValueError(
                f"Donnees manquantes pour reservation #{d['id']} — "
                f"salle_id={d['salle_id']}, "
                f"responsable_id={d['responsable_id']}"
            )
        r = Reservation(
            salle=salle,
    ...
    def charger_tout(self) -> dict:
        """
        Charge toutes les entites depuis les fichiers JSON.
        Retourne {'utilisateurs': [...], 'salles': [...], 'reservations': [...]}.

        Met a jour Reservation._compteur pour que les nouvelles reservations
        creees en memoire aient des ids superieurs au max enregistre en JSON.
        """
